 ...
 * 项目名称: demo
 * <p>
 * 类的描述: 垂直公告栏, 整个实现无限轮播, 注意, 注意item 至少需要2 个
 *      TODO lz 若是需要实现轮播内容, 只需要在外层设置一个比较小的布局, 即可实现内容轮播, 要求匀速则需在下方设置 animator .setInterpolator(new LinearInterpolator());
...
public class VerticalMarqueeLayout<T> extends ViewGroup {
    //滚动间隔时间 和滚动动画时间, 若是要实现无限无间循环轮播, 则两个数值需要相等, item 间隔时间 = 滚动间隔时间 - 滚动动画时间
    public static final int DURATION_SCROLL = 3000;
    public static final int DURATION_ANIMATOR = 3000;
    //实体集合和子控件集合
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if(current == beans.size() - 1){
                    ...
                    if(builder != null){
                        ...
                        // TODO lz 定制, 单个时, 解决数组越界问题
                        builder.builder(view, beans.get(current + 1 < beans.size() ? current + 1 : current));
                    }
                }else{
                    if(builder != null){
                        builder.builder(views.get(0), beans.get(current));
    ...
    public abstract class OnItemBuilder{
        ...
        private void measure(View view){
            int widthMeasure = MeasureSpec.makeMeasureSpec(width - getPaddingLeft() - getPaddingRight(), MeasureSpec.EXACTLY);
