package com.lib.lzlin.api.utils.takePhotoUtils.camare;
...
public class CameraPreview extends SurfaceView implements
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		try {
			...
			Log.e(TAG, "camera == null ? " + (camera == null) );
			Log.e(TAG, "holder == null ? " + (holder == null) );
			// TODO lz 当手机没有相机时, 获取到的相机对象时空的, 会报报空指针错误, 需要判空, 并作出提示
	...
	OnTouchListener onTouchListener = new OnTouchListener() {
		...
		public boolean onTouch(View v, MotionEvent event) {
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				...
				boolean isSuccessFocusOnTouch = focusOnTouch(event); // 是否可以取得焦点
				return isSuccessFocusOnTouch;
			}
			return true;
		}
	};
	 ...
	 * @param event
	 */
	public boolean focusOnTouch(MotionEvent event) {
		...
		// TODO lz 定制, 用于调试手机没有摄像头时, 返回错误提示, 否则会报空指针异常
		if (camera == null) {
			return false;
		}
