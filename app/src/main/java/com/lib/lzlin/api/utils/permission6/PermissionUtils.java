 ...
 * 项目名称: Lib-lz
 * <p>
 * 类的描述: Android 6.0 权限管理工具类 使用这个utils 工具类的情况,
 *     1. 首先需要在 build.gradle 文件中设置 targetSdkVersion >= 23, 才需要动态申请, 否则只需设置 targetSdkVersion 最大只能设置为22.
 *     2. 在AndroidManifest.xml 中申请你需要的权限, 包括普通权限和需要申请的特殊权限.
 *     3. 开始动态申请权限, 3 步走计划:
 *      （1）检查是否由此权限checkSelfPermission()，如果已经开启，则直接做你想做的。
 *      （2）如果未开启，则判断是否需要向用户解释为何申请权限shouldShowRequestPermissionRationale。
 *      （3）如果需要（即返回true），则可以弹出对话框提示用户申请权限原因，用户确认后申请权限requestPermissions()，
 *          如果不需要（即返回false），则直接申请权限requestPermissions()。
 * 创建人: lz - Administrator
 * 创建时间:  2017/5/2 13:50
 * 修改人: lz - Administrator
 * 修改备注:
 *  致谢参考网址: http://blog.csdn.net/sun18612393433/article/details/53283235
 *      普通权限（normal）：这个权限类型并不直接威胁到用户的隐私，可以直接在manifest清单里注册，系统会帮我们默认授权的。
 *      危险权限 （dangerous）：这个可以直接给app访问用户一些敏感的数据，不仅需要在manifest清单里注册，同时在使用的时候，需要向系统请求授权。
 *      危险权限的特点： 危险权限是按组分配的，只要同个组的某个权限被同意后，组中的其他权限也会被默认同意
 */
